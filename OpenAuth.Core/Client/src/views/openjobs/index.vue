          ...
          <el-table-column align="center" :label="'操作'" width="80" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button size="mini" :type="`${scope.row.status === 0 ? 'success' : 'danger'}`"
                @click="handleModifyStatus(scope.row)">{{scope.row.status === 0 ? '启用' : '停止'}}</el-button>
            </template>
          </el-table-column>
        </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      handleModifyStatus(row) { // 模拟修改状态
        ...
        this.$confirm(`确认${status === 0 ? '停止' : '启动'}定时任务"${row.jobName}"?`, '提示', {
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              openJobs.changeStatus({ id: row.id, status: status }).then(() => {
                this.$message.success(`"${row.jobName}"${status === 0 ? '已停止' : '已启动'}`)
        ...
        this.$confirm(`确认删除?`, '提示', {
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              instance.confirmButtonLoading = true
              instance.confirmButtonText = '执行中...'
              openJobs.del(rows.map(u => u.id)).then(() => {
                this.$notify({
                  title: '成功',
                  message: '删除成功',
                  type: 'success',
