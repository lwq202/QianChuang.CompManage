...
var Utils = /** @class */ (function () {
    ...
    Utils.prototype.generateUploadComponent = function (h, option, vm) {
        var _t = vm || this;
        var style = option.style, props = option.props, slot = option.slot, directives = option.directives, key = option.key, events = option.events, label = option.label, mapKey = option.mapKey;
        var compData = {
            directives: directives,
            on: events || {},
            props: __assign(__assign(__assign({}, displayProp(_t)), { action: actionApi, disabled: _t.status === _t.$actionType.detail, limit: 1 }), props),
            style: style,
        };
        // 上传钩子
        if (!compData.props.onSuccess) {
            compData.props.onSuccess = function (res, file, fileList) {
                ...
                if (compData.props.limit > 1) {
                    fileIds = fileList.map(function (item) { return item.response ? item.response.Id : item.Id; });
                }
        ...
        if (value) {
            if (_.isArray(value)) {
                dataFiles = value.map(function (item) {
        ...
        var defaultSlot = <el-button type="primary">{_t.$t('form.clickUpload')}</el-button>;
        if (slot) {
